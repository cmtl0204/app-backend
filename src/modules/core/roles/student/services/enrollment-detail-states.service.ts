...
export class EnrollmentDetailStatesService {
  ...
  async removeRequestSent(payload: EnrollmentDetailStateEntity[]): Promise<boolean> {
    const requestSent = payload.find(
      (enrollmentState) => enrollmentState.state.code === CatalogueEnrollmentStateEnum.REQUEST_SENT,
    );
  ...
  async removeApproved(payload: EnrollmentDetailStateEntity[]): Promise<boolean> {
    const approved = payload.find(
      (enrollmentState) => enrollmentState.state.code === CatalogueEnrollmentStateEnum.APPROVED,
    );
  ...
  async removeRejected(enrollmentStates: EnrollmentDetailStateEntity[]): Promise<boolean> {
    const rejected = enrollmentStates.find(
      (enrollmentState) => enrollmentState.state.code === CatalogueEnrollmentStateEnum.REJECTED,
    );
